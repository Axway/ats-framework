	...
	public void connect(String user, String password, String host, int port, String privateKey,
		...
		try {
				...
				public Config getConfig(String host) {
					Config config = new Config() {
						...
						public String getUser() {
							return JschSshClient.this.user;
						}

						@Override
						public int getPort() {
							return JschSshClient.this.port;
						}

						@Override
						public String getHostname() {
							return JschSshClient.this.host;
						}
					};
					return config;
				}
			...
			throw new JschSshClientException(
					e.getMessage() + "; Connection parameters are: user '" + user + "' at " + host + " on port " + port,
					e);
		}
	}
						...
						while (this.is.available() > 0) {
							...
							if (this.streamContent.length() > MAX_STRING_SIZE) {
								dataToLeave = this.streamContent
										.substring(this.streamContent.length() - MAX_STRING_SIZE);
			...
			synchronized (this.streamContent) {
				...
				if (!this.readFinished) {
					throw new RuntimeException(
							"The " + this.type + " was not read in " + READ_TIMEOUT / 1000 + " seconds");
				}
				return this.streamContent.toString();
			}
		}
