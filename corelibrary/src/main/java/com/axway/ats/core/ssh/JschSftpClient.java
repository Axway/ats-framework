...
public class JschSftpClient {
	...
	private static boolean isVerbose = AtsSystemProperties
			.getPropertyAsBoolean(AtsSystemProperties.CORE__JSCH_VERBOSE_MODE, false);
	/*
	 ...
	 *            the specific port to use
	 * @param privateKey
	 *            private key location. For example: ~/.ssh/id_rsa
	 * @param privateKeyPassword
	 *            private key passphrase (or null if it hasn't)
	 */
				...
				public Config getConfig(String host) {
					Config config = new Config() {
						...
						public String getUser() {
							return JschSftpClient.this.user;
						}

						@Override
						public int getPort() {
							return JschSftpClient.this.port;
						}

						@Override
						public String getHostname() {
							return JschSftpClient.this.host;
						}
					};
					return config;
				}
	 ...
	 * @param directoryPath
	 *            directory path
	 * @return {@link List} of {@link FileEntry} objects corresponding with the
	 *         target directory file and folder entries.
	 */
	...
	public void uploadDirectory(String sourceDir, String destDir, boolean createParentDirs) {
		...
		if (!srcDir.isDirectory()) {
			...
			throw new JschSftpClientException(
					"The local directory '" + sourceDir + "' is not a Directory or doesn't exist");
		}
	 ...
	 *            the directory path
	 * @param isRecursive
	 *            whether to delete directory entries recursively
	 */
	public void removeRemoteDirectory(String directoryPath, boolean isRecursive) {

		if (isVerbose) {
			log.debug("Delete remote directory " + directoryPath + (isRecursive ? " recursively" : ""));
		}
		...
		try {
			...
			throw new JschSftpClientException(
					"Error while deleting directory '" + directoryPath + "'" + (isRecursive ? " recursively" : ""), e);
		}
	}
	 ...
	 * @param directoryPath
	 *            the directory path
	 * @param preservedPaths
	 *            a list with preserved files or folders. If the path ends with '/'
	 *            it is preserved directory, not a file
	 */
	...
	private FileEntry getFileEntry(LsEntry lsEntry, String parentPath) {
		...
		FileEntry fileEntry = new FileEntry(lsEntry.getFilename(),
				IoUtils.normalizeUnixDir(parentPath) + lsEntry.getFilename(), lsEntry.getAttrs().isDir());
	...
	/**
	 *
	 * LsEntry selector, which collects only the internal files and folders without
	 * the current directory entry ('.') and its parent ('..')
	 */
