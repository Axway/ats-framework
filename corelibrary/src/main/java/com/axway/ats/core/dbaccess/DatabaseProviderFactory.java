...
public class DatabaseProviderFactory {
    			...
    			if (doc != null) {
    				...
    				for (int i = 0; i < children.getLength(); i++) {
    					...
    					if ("provider".equals(child.getNodeName())) {
    						...
    						dbProviders.put(alias, new String[] { dbConnectionClassName, dbProviderClassName });
    					}
    				}
    			}
    		}
	...
	public static synchronized DbProvider getDatabaseProvider(String dbType, String dbHost, String dbName,
			String dbUser, String dbPass, int dbPort, Map<String, Object> customProperties) {
		...
		case DbConnMySQL.DATABASE_TYPE:
			dbProvider = new MysqlDbProvider((DbConnMySQL) createDbConnection(dbType, dbHost,
					dbName, dbUser, dbPass, customProperties));
			break;
	...
	private static DbProvider loadCustomDbProvider(String className, DbConnection dbConnection) {
        ...
        try {
            constructorDbProvider = new MethodFinder(dbProviderClass)
                    .findConstructor(new Class[] { DbConnection.class });
    ...
    private static DbConnection loadDbConnection(String className, String dbType, String dbHost, String dbName,
	                                             String dbUser, String dbPass, int dbPort, Map<String, Object> customProperties) {
        ...
        try {
            constructor = new MethodFinder(dbConnectionClass).findConstructor(
        ...
        try {
            dbConnection = (DbConnection) constructor
                    .newInstance(new Object[] { dbHost, dbName, dbUser, dbPass, customProperties });
        } catch (InstantiationException | IllegalAccessException | IllegalArgumentException
                | InvocationTargetException e) {
            throw new DbException("Unable to get database provider for type '" + dbType + "'", e);
        }
	 ...
	 *            the login password
	 * @param customProperties
	 *            a set of custom properties for the connection
	 * @return database connection descriptor
	 */
	public static DbConnection createDbConnection(String dbType, String host, String database, String user,
